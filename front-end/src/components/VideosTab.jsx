...
export default function VideosTab({ darkMode, cardBg, textClass, secondaryText, borderClass, studyTopic, setStudyTopic }) {
  const { videos, loading, add, remove, update } = useVideos();
  ...
  const saveEdit = async (id) => {
    ...
    try {
      await update(id, {
        ...
        title: editTitle.trim() || 'Saved Video',
      });
      cancelEdit();
    } catch (err) {
  ...
  const getVideoId = (url) => extractYouTubeID(url);

  // Get YouTube thumbnail URL for a given video URL
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          [...Array(3)].map((_, i) => (
            <div
              key={i}
              className={`${cardBg} rounded-xl shadow-lg border ${borderClass} animate-pulse`}
            >
            ...
            return (
              <div key={video.id} className={`${cardBg} rounded-xl shadow-lg overflow-hidden border ${borderClass}`}>
                {editingId === video.id ? (
                  ...
                  <div className="p-4">
                    <input 
                      className={`w-full px-3 py-2 rounded-lg border ${borderClass} mb-2 ${darkMode ? 'bg-gray-700' : 'bg-white'}`} 
                      value={editUrl} 
                  ...
                  <>
                    ...
                    <div className={`aspect-video ${darkMode ? 'bg-gray-700' : 'bg-gray-200'} flex items-center justify-center`}>
                      {thumbnail ? (
                        <img src={thumbnail} alt={`Thumbnail for ${video.title || 'video'}`} className="object-cover w-full h-full" />
                      ) : (
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="font-semibold mb-2">{video.title || 'Saved Video'}</h3>
                      <p className={`text-sm ${secondaryText} mb-3`}>{video.topic ? video.topic.toLowerCase() : 'this topic'}</p>
                      {/* Action buttons */}
                      <div className="flex gap-2">
                        <a href={video.video_url} target="_blank" rel="noreferrer" className="px-3 py-1 rounded bg-indigo-600 text-white">Open</a>
